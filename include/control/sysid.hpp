 ...
 * 
 * @see Chapter 9: System Identification — From Measured Data to Mathematical Models
 * 
...
inline double compute_fit(const std::vector<double>& y,
    ...
    if (y.size() != y_hat.size() || y.empty()) return 0.0;
    ...
    if (norm_ref < 1e-15) return 100.0;  // Constant output perfectly matched
...
inline int detect_step_order(const std::vector<double>& y) {
    ...
    if (std::abs(y_final) < 1e-15) return 1;
 ...
 */
inline FirstOrderIDResult id_step_first_order(
    const std::vector<double>& t,
    const std::vector<double>& y,
    double u_step = 1.0
) {
    if (t.size() != y.size() || t.size() < 3) {
        throw std::runtime_error("id_step_first_order: need at least 3 data points");
    }
    ...
    double y_ss = 0.0;
    for (size_t i = start_ss; i < y.size(); ++i) y_ss += y[i];
    ...
    for (int iter = 0; iter < 10; ++iter) {
        ...
        // Ensure tau > 0
        if (tau_est <= 0) tau_est = 0.001;
        
        if (std::abs(dK) < 1e-10 * std::abs(K_est) &&
            std::abs(dtau) < 1e-10 * std::abs(tau_est)) break;
    }
 ...
 */
inline SecondOrderIDResult id_step_second_order(
    const std::vector<double>& t,
    const std::vector<double>& y,
    double u_step = 1.0
) {
    if (t.size() != y.size() || t.size() < 5) {
        throw std::runtime_error("id_step_second_order: need at least 5 data points");
    }
    ...
    if (result.tp > 0 && result.zeta < 1.0) {
        ...
        for (size_t i = 0; i < y.size() - 1; ++i) {
            if (y[i] <= target_10 && y[i + 1] >= target_10) t_10 = t[i];
            if (y[i] <= target_90 && y[i + 1] >= target_90) { t_90 = t[i]; break; }
        }
        double tr = t_90 - t_10;
 ...
 */
inline StepIDParams id_step_auto(
    const std::vector<double>& t,
    const std::vector<double>& y,
    double u_step = 1.0
) {
...
inline BodeFitResult id_bode(
    ...
    int rel_degree = std::max(1, std::min(max_order, 
                     static_cast<int>(std::round(-slope / 20.0))));
...
inline ARXModel id_arx(
    ...
    model.a.resize(na);
    model.b.resize(nb);
    for (int i = 0; i < na; ++i) model.a[i] = -theta(i, 0);  // Note: Φ stores -y
    for (int i = 0; i < nb; ++i) model.b[i] = theta(na + i, 0);
...
inline TransferFunction arx_to_tf(const ARXModel& model) {
    ...
    auto binomial_expand = [&](int power_plus, int power_minus) -> std::vector<double> {
        ...
        for (int k = 0; k <= power_plus; ++k) {
            // C(p,k) * 2^(p-k) * (sT)^k
            double binom = 1.0;
            for (int j = 0; j < k; ++j) binom *= (double)(power_plus - j) / (j + 1);
            poly_plus[power_plus - k] = binom * std::pow(2.0, power_plus - k) * std::pow(T, k);
        }
        ...
        for (int k = 0; k <= power_minus; ++k) {
            double binom = 1.0;
            for (int j = 0; j < k; ++j) binom *= (double)(power_minus - j) / (j + 1);
            double sign = (k % 2 == 0) ? 1.0 : -1.0;
            poly_minus[power_minus - k] = binom * std::pow(2.0, power_minus - k) * std::pow(T, k) * sign;
        }
    ...
    for (int k = 0; k < n_num; ++k) {
        auto expanded = binomial_expand(n_den - 1 - k - (n_den - n_num), k + (n_den - n_num));
        // Adjust indices
    ...
    if (std::abs(lead) > 1e-15) {
        for (auto& v : num_result) v /= lead;
        for (auto& v : den_result) v /= lead;
    }
 ...
 */
inline std::vector<double> arx_predict(
    ...
    for (int k = start; k < N; ++k) {
        ...
        for (int i = 0; i < model.nb; ++i) {
            ...
            if (u_idx >= 0) val += model.b[i] * u[u_idx];
        }
        y_hat[k] = val;
    }
 ...
 */
inline std::vector<double> arx_simulate(
    const ARXModel& model,
    const std::vector<double>& u
) {
 ...
 */
inline std::pair<std::vector<double>, std::vector<double>> generate_prbs(
    int n_bits = 7,
 ...
 * @return std::pair<time_vector, signal_vector>
 */
inline std::pair<std::vector<double>, std::vector<double>> generate_chirp(
    double f0,
    double f1,
    double duration,
    double Ts = 0.001,
    ...
    for (int k = 0; k < N; ++k) {
        t[k] = k * Ts;
        double phase = 2.0 * M_PI * (f0 * t[k] + (f1 - f0) / (2.0 * duration) * t[k] * t[k]);
 ...
 * @return std::pair<time_vector, signal_vector>
 */
inline std::pair<std::vector<double>, std::vector<double>> generate_multisine(
    ...
    for (int i = 0; i < N; ++i) {
        t[i] = i * Ts;
        for (int k = 0; k < K; ++k) {
            signal[i] += amplitude * std::sin(2.0 * M_PI * frequencies[k] * t[i] + phases[k]);
        }
    }
 ...
 * @return std::pair<time_vector, signal_vector>
 */
inline std::pair<std::vector<double>, std::vector<double>> generate_white_noise(
    int N,
    double Ts = 0.01,
    double std_dev = 1.0,
    unsigned int seed = 0
...
inline std::vector<double> autocorrelation(
    ...
    if (max_lag < 0) max_lag = std::min(N / 4, 50);
 ...
 */
inline std::vector<double> crosscorrelation(
    const std::vector<double>& x,
    const std::vector<double>& y,
    int max_lag = -1
) {
    ...
    for (int k = 0; k < N; ++k) { mean_x += x[k]; mean_y += y[k]; }
    mean_x /= N; mean_y /= N;
 ...
 */
inline ValidationResult validate_model(
    const ARXModel& model,
    const std::vector<double>& u,
    const std::vector<double>& y
) {
 ...
 * @return ValidationResult
 */
inline ValidationResult validate_tf(
    ...
    if (is_step && N > 1) {
        ...
        for (int k = 0; k < N; ++k) {
            // Shift input buffer
            for (int j = n_num - 1; j > 0; --j) u_buf[j] = u_buf[j - 1];
            u_buf[0] = (k < static_cast<int>(u.size())) ? u[k] : 0.0;
            
            // Compute output
            double out = 0.0;
            for (int j = 0; j < n_num; ++j) out += num_c[j] * u_buf[j];
            for (int j = 1; j < n_den; ++j) out -= den_c[j] * y_buf[j - 1];
            if (std::abs(den_c[0]) > 1e-15) out /= den_c[0];
            
            // Shift output buffer
            for (int j = n_den - 1; j > 0; --j) y_buf[j] = y_buf[j - 1];
            y_buf[0] = out;
 ...
 */
inline InformationCriteria compute_aic_bic(
    double sigma2, int num_params, int N
) {
 ...
 */
inline std::pair<std::vector<double>, std::vector<double>> coherence(
    const std::vector<double>& u,
    const std::vector<double>& y,
    double Ts = 0.01,
    int n_seg = 8
 ...
 * @return Output signal vector
 */
inline std::vector<double> lsim(
    const TransferFunction& G,
    const std::vector<double>& t,
    const std::vector<double>& u
) {
    if (t.size() < 2 || t.size() != u.size()) {
        throw std::runtime_error("lsim: need at least 2 time points; t and u must match");
    }
...
inline void print_arx(const ARXModel& model) {
    ...
    std::cout << "  y[k] = ";
    for (int i = 0; i < model.na; ++i) {
        if (i > 0) std::cout << (model.a[i] >= 0 ? " + " : " - ");
        else if (model.a[i] < 0) std::cout << "-";
    ...
    std::cout << "\nResidual variance: " << std::setprecision(6) << model.sigma2 << std::endl;
...
inline void print_validation(const ValidationResult& val) {
    ...
    std::cout << "FIT:  " << std::fixed << std::setprecision(1) 
              << val.fit_percent << "%" << std::endl;
    ...
    std::cout << "Residuals uncorr. with input: "
              << (val.residuals_uncorr_with_input ? "YES ✓" : "NO ✗") << std::endl;
    std::cout << "======================================" << std::endl;
}
...
inline void print_step_id(const FirstOrderIDResult& r) {
    std::cout << "========== Step Response ID (1st Order) ==========" << std::endl;
    std::cout << "  DC Gain K   = " << std::setprecision(4) << r.K << std::endl;
    std::cout << "  Time const τ = " << std::setprecision(4) << r.tau << " s" << std::endl;
    std::cout << "  G(s) = " << r.K << " / (" << r.tau << "s + 1)" << std::endl;
    std::cout << "  FIT = " << std::fixed << std::setprecision(1) 
              << r.fit_percent << "%" << std::endl;
    std::cout << "==================================================" << std::endl;
}

inline void print_step_id(const SecondOrderIDResult& r) {
    std::cout << "========== Step Response ID (2nd Order) ==========" << std::endl;
    std::cout << "  DC Gain K     = " << std::setprecision(4) << r.K << std::endl;
    std::cout << "  Nat. freq ωn  = " << std::setprecision(4) << r.wn << " rad/s" << std::endl;
    std::cout << "  Damping ζ     = " << std::setprecision(4) << r.zeta << std::endl;
    std::cout << "  Overshoot Mp  = " << std::setprecision(1) << (r.Mp * 100) << "%" << std::endl;
    std::cout << "  Peak time tp  = " << std::setprecision(4) << r.tp << " s" << std::endl;
    std::cout << "  Settling ts   ≈ " << std::setprecision(4) << r.ts << " s" << std::endl;
    std::cout << "  FIT = " << std::fixed << std::setprecision(1) 
              << r.fit_percent << "%" << std::endl;
    std::cout << "==================================================" << std::endl;
}
