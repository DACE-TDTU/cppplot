...
public:
    ...
    TransferFunction(const Polynomial& n, const Polynomial& d) 
        : num(n), den(d) {
        if (den.isZero()) {
    ...
    TransferFunction(std::initializer_list<double> n, std::initializer_list<double> d)
        : num(n), den(d) {
        if (den.isZero()) {
            throw std::runtime_error("Denominator cannot be zero");
        }
    ...
    double gain() const {
        return num.leading() / den.leading();
    }
    ...
    std::complex<double> evalS(std::complex<double> s) const {
        return eval(s);
    }
    ...
    double mag_dB(double omega) const {
        double m = mag(omega);
        if (m < 1e-20) return -400;  // Avoid log(0)
    ...
    bool isMarginallyStable() const {
        ...
        for (const auto& pole : p) {
            if (pole.real() > 1e-10) return false;  // Unstable
            if (std::abs(pole.real()) < 1e-10) hasJwPole = true;
        }
        return hasJwPole;
    }
    ...
    double stepResponse(double t) const {
        if (t < 0) return 0;
        if (t == 0) return 0;  // Assuming strictly proper system
        ...
        if (n == 2) {
            ...
            if (zeta < 1.0 - 1e-10) {
                ...
                return K * (1 - std::exp(-zeta * wn * t) * std::sin(wd * t + phi) / std::sqrt(1 - zeta * zeta));
            } else if (zeta > 1.0 + 1e-10) {
                // Overdamped
                double s1 = -zeta * wn + wn * std::sqrt(zeta * zeta - 1);
                double s2 = -zeta * wn - wn * std::sqrt(zeta * zeta - 1);
                return K * (1 + (s2 * std::exp(s1 * t) - s1 * std::exp(s2 * t)) / (s1 - s2));
            } else {
                // Critically damped
        ...
        auto state_derivative = [&](const std::vector<double>& state) -> std::vector<double> {
        ...
        for (int step = 0; step < steps; ++step) {
            ...
            for (int i = 0; i < n; ++i) x_temp[i] = x[i] + 0.5 * dt * k1[i];
            std::vector<double> k2 = state_derivative(x_temp);
            
            for (int i = 0; i < n; ++i) x_temp[i] = x[i] + 0.5 * dt * k2[i];
            std::vector<double> k3 = state_derivative(x_temp);
            
            for (int i = 0; i < n; ++i) x_temp[i] = x[i] + dt * k3[i];
     ...
     */
    std::pair<std::vector<double>, std::vector<double>> stepResponseData(
        double t_final = 10.0, 
        int num_points = 500
    ) const {
 ...
 */
inline TransferFunction tf_second_order(double wn, double zeta, double K = 1.0) {
    double wn2 = wn * wn;
 ...
 */
inline TransferFunction feedback(
    const TransferFunction& G, 
    const TransferFunction& H = TransferFunction(1.0),
    int sign = -1
) {
...
inline TransferFunction parallel(const std::vector<TransferFunction>& systems) {
    if (systems.empty()) return TransferFunction(0.0);
