 ...
 */
inline std::vector<double> logspace(double start, double stop, size_t num = 50, double base = 10.0) {
 ...
 */
inline std::vector<double> arange(double start, double stop, double step = 1.0) {
    std::vector<double> result;
    if (step > 0) {
 ...
 */
inline std::vector<double> random(size_t n, double min = 0.0, double max = 1.0) {
 ...
 */
inline std::vector<double> randn(size_t n, double mean = 0.0, double stddev = 1.0) {
    static std::mt19937 gen(std::random_device{}());
...
inline HistogramResult histogram(const std::vector<double>& data, int bins = 10,
    ...
    if (data.empty() || bins <= 0) return result;
    
    double dataMin = std::isnan(minVal) ? *std::min_element(data.begin(), data.end()) : minVal;
    double dataMax = std::isnan(maxVal) ? *std::max_element(data.begin(), data.end()) : maxVal;
 ...
 */
inline std::vector<double> niceTicks(double min, double max, int maxTicks = 10) {
 ...
 */
inline std::vector<double> niceLogTicks(double min, double max, int maxTicks = 10) {
    std::vector<double> ticks;
...
public:
    CoordinateTransform() : flipY_(true) {}
    
    CoordinateTransform(const Rect& dataRect, const Rect& pixelRect, bool flipY = true)
        : dataRect_(dataRect), pixelRect_(pixelRect), flipY_(flipY) {}
    ...
    Point dataToPixel(double x, double y) const {
        double px = pixelRect_.x + (x - dataRect_.x) / dataRect_.width * pixelRect_.width;
        double py;
        if (flipY_) {
            py = pixelRect_.y + pixelRect_.height - (y - dataRect_.y) / dataRect_.height * pixelRect_.height;
        } else {
            py = pixelRect_.y + (y - dataRect_.y) / dataRect_.height * pixelRect_.height;
        }
        return Point(px, py);
    }
    
    Point dataToPixel(const Point& p) const {
        return dataToPixel(p.x, p.y);
    }
    
    Point pixelToData(double px, double py) const {
        double x = dataRect_.x + (px - pixelRect_.x) / pixelRect_.width * dataRect_.width;
        double y;
        if (flipY_) {
            y = dataRect_.y + (pixelRect_.y + pixelRect_.height - py) / pixelRect_.height * dataRect_.height;
        } else {
            y = dataRect_.y + (py - pixelRect_.y) / pixelRect_.height * dataRect_.height;
        }
        return Point(x, y);
    }
    
    Point pixelToData(const Point& p) const {
        return pixelToData(p.x, p.y);
    }
