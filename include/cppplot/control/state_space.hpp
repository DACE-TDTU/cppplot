...
public:
  ...
  StateSpace(const Matrix &A_, const Matrix &B_, const Matrix &C_,
             const Matrix &D_)
      : A(A_), B(B_), C(C_), D(D_) {
    n_states = A.rows;
    n_inputs = B.cols;
    ...
    if (B.rows != n_states)
      throw std::runtime_error("B row count must match A");
    if (C.cols != n_states)
      throw std::runtime_error("C column count must match A");
  ...
  std::string toString() const {
    ...
    oss << "State-Space System (n=" << n_states << ", m=" << n_inputs
        << ", p=" << n_outputs << ")\n\n";
    oss << "A =\n" << A.toString() << "\n\n";
    oss << "B =\n" << B.toString() << "\n\n";
    oss << "C =\n" << C.toString() << "\n\n";
...
/**
 * @brief Simulate state-space response with arbitrary input (MIMO)
 * @param sys State-space system
 * @param u Input signal (vector of input vectors, one per time step: N x m)
 * @param t Time vector
 * @param x0 Initial state (optional)
 * @return Output vector y (N x p) and state history (N x n)
 */
inline std::pair<std::vector<std::vector<double>>,
                 std::vector<std::vector<double>>>
lsim(const StateSpace &sys, const std::vector<std::vector<double>> &u,
     const std::vector<double> &t, const std::vector<double> &x0 = {}) {
  size_t N = t.size();
  ...
  for (size_t k = 0; k < N; ++k) {
    ...
    if (k < N - 1) {
      double dt = t[k + 1] - t[k];
      const auto &u_k = u[k];
      const auto &u_k1 = u[k + 1];
      std::vector<double> u_mid(m);
      for (size_t j = 0; j < m; ++j) {
        u_mid[j] = 0.5 * (u_k[j] + u_k1[j]); // Linear input interpolation
      }
...
/**
 * @brief Simulate SISO state-space response with arbitrary input (backward
 * compatibility)
 */
inline std::pair<std::vector<double>, std::vector<std::vector<double>>>
lsim(const StateSpace &sys, const std::vector<double> &u,
     const std::vector<double> &t, const std::vector<double> &x0 = {}) {
  size_t N = t.size();
  if (sys.n_inputs != 1 || sys.n_outputs != 1) {
    throw std::runtime_error("1D lsim overload only supports SISO systems. Use "
                             "the 2D vector overload for MIMO.");
  }

  // Convert 1D input to 2D
  std::vector<std::vector<double>> u_mimo(N, std::vector<double>(1));
  for (size_t k = 0; k < N; ++k) {
    u_mimo[k][0] = u[k];
  }

  auto result_mimo = lsim(sys, u_mimo, t, x0);

  // Convert 2D output back to 1D
  std::vector<double> y_siso(N);
  for (size_t k = 0; k < N; ++k) {
    y_siso[k] = result_mimo.first[k][0];
  }

  return std::make_pair(y_siso, result_mimo.second);
}
...
ss_step(const StateSpace &sys, double T = 10.0, int N = 500) {
  if (sys.n_inputs != 1 || sys.n_outputs != 1) {
    throw std::runtime_error("ss_step currently only provides 1D output. Use "
                             "lsim for MIMO step responses.");
  }
 ...
 */
inline std::pair<std::vector<double>, std::vector<double>>
initial(const StateSpace &sys, const std::vector<double> &x0, double T = 10.0,
        int N = 500) {
  if (sys.n_inputs != 1 || sys.n_outputs != 1) {
    throw std::runtime_error("initial currently only provides 1D output. Use "
                             "lsim for MIMO initial responses.");
  }
...
inline void ssinfo(const StateSpace &sys) {
  ...
  std::cout << "  Controllable: " << (sys.isControllable() ? "Yes" : "No")
            << std::endl;
  std::cout << "  Observable: " << (sys.isObservable() ? "Yes" : "No")
            << std::endl;
