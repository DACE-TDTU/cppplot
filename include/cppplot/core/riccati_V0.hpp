    ...
    std::function<bool(std::complex<double>)> select)
{
    ...
    // ── Swap two adjacent blocks at positions (pos, pos+si) ─────────
    auto swap_blocks = [&](int pos, int si, int sj) {
        ...
        // Build local orthogonal transformation Z  (sz × sz)
        Eigen::MatrixXd Z = Eigen::MatrixXd::Identity(sz, sz);
        double t12_norm = T12.norm();

        if (t12_norm > eps) {
            // Bai-Demmel (1993): solve T11*X - X*T22 = -T12  (si×sj)
            // sylvester(A, B, C): AX + XB = C
            //   T11*X + X*(-T22) = -T12
            ...
            Eigen::MatrixXd X = to_eigen(mX);  // si × sj

            // W = [ I_si   X  ]   (upper block form)
            //     [  0    I_sj]
            Eigen::MatrixXd W = Eigen::MatrixXd::Identity(sz, sz);
            W.block(0, si, si, sj) = X;

            // Permutation P = [ 0   I_sj ]  (puts T22 first)
            //                 [ I_si  0  ]
            Eigen::MatrixXd P_perm = Eigen::MatrixXd::Zero(sz, sz);
            P_perm.block(0,  si, sj, sj) = Eigen::MatrixXd::Identity(sj, sj);
            P_perm.block(sj,  0, si, si) = Eigen::MatrixXd::Identity(si, si);

            // QR of P*W → Z (orthogonal factor)
            Eigen::HouseholderQR<Eigen::MatrixXd> qrW(P_perm * W);
            Z = qrW.householderQ() * Eigen::MatrixXd::Identity(sz, sz);
        } else {
            // T12 ≈ 0 — simple block permutation
            // P = [ 0_{sj×si}   I_sj ]
            //     [ I_si        0    ]
        ...
        // Apply similarity: T[pos:pos+sz, :] ← Z^T * T[pos:pos+sz, :]
        ...
        // U[:, pos:pos+sz] ← U[:, pos:pos+sz] * Z
        Eigen::MatrixXd Ucols = U.middleCols(pos, sz);
        U.middleCols(pos, sz) = Ucols * Z;

        // Zero out sub-diagonal elements that should be zero
        double tol_zero = eps * T.lpNorm<Eigen::Infinity>();
        for (int r = pos + 1; r < pos + sz; ++r)
            for (int c = pos; c < r && c < pos + sz; ++c)
