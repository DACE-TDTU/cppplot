                   ...
                   const Matrix& Q, const Matrix& R)
{
    // ── Dimension checks ──────────────────────────────────────────
    size_t n = A.rows;
    if (A.cols != n) throw std::runtime_error("dare: A must be square");
    if (B.rows != n) throw std::runtime_error("dare: B must have n rows");
    if (Q.rows != n || Q.cols != n) throw std::runtime_error("dare: Q must be n×n");
    if (R.rows != B.cols || R.cols != B.cols) throw std::runtime_error("dare: R must be m×m");
    ...
    Eigen::MatrixXd eRinv = ldltR.solve(Eigen::MatrixXd::Identity(static_cast<int>(m), static_cast<int>(m)));
    ...
    if (!A_invertible) {
        // ── Fallback for Singular A: Structure-Preserving Doubling Algorithm ──
        // Lách hoàn toàn việc tính A^{-1}, hội tụ bậc 2.
        Eigen::MatrixXd Ak = eA;
        Eigen::MatrixXd Bk = eB * eRinv * eB.transpose();
        Eigen::MatrixXd Qk = eQ;
        ...
        for (int iter = 0; iter < max_iter; ++iter) {
            Eigen::MatrixXd Mk = (I + Bk * Qk).inverse();

            Eigen::MatrixXd Ak_next = Ak * Mk * Ak;
            Eigen::MatrixXd Bk_next = Bk + Ak * Mk * Bk * Ak.transpose();
            Eigen::MatrixXd Qk_next = Qk + Ak.transpose() * Qk * Mk * Ak;

            double diff = (Qk_next - Qk).norm();
            Ak = Ak_next;
            Bk = Bk_next;
            Qk = Qk_next;

            // Symmetrize to prevent numerical drift
            Qk = (Qk + Qk.transpose()) * 0.5;
            Bk = (Bk + Bk.transpose()) * 0.5;

            if (diff < tol * std::max(1.0, Qk.norm())) {
                converged = true;
                break;
            }
        }
        ...
        if (!converged)
            throw std::runtime_error("dare: Doubling algorithm failed to converge");

        Eigen::MatrixXd P_eig = Qk; // Qk chính là nghiệm P
        double res = dare_residual(eA, eB, eQ, eR, P_eig);
        if (res > 1e-6 * (1.0 + P_eig.norm()))
            throw std::runtime_error("dare: solution inaccurate (residual = " + std::to_string(res) + ")");

        return from_eigen(P_eig);
    }

    // ── Invertible A: Symplectic Schur method ─────────────────────
    int N = 2 * static_cast<int>(n);
    ...
    Eigen::MatrixXd T = schurZ.matrixT();
    Eigen::MatrixXd U = schurZ.matrixU();

    auto unit_disk_select = [](std::complex<double> z) { return std::abs(z) < 1.0; };
    ...
    while (i < N) {
        int si = schur_block_size(T, i);
        if (unit_disk_select(schur_block_eig(T, i))) n_stable += si;
        i += si;
    }
    if (n_stable != static_cast<int>(n))
        throw std::runtime_error("dare: symplectic matrix does not have exactly n stable eigenvalues");

    reorder_schur_eigen(T, U, unit_disk_select);

    Eigen::MatrixXd U11 = U.topLeftCorner(static_cast<int>(n), static_cast<int>(n));
    Eigen::MatrixXd U21 = U.bottomLeftCorner(static_cast<int>(n), static_cast<int>(n));

    double cond_U11 = U11.norm() * U11.inverse().norm();
    if (cond_U11 > 1e12)
        throw std::runtime_error("dare: U11 is ill-conditioned");
