/**
 * @file benchmark_qp_solvers_V3.cpp
 ...
 *
 * Compile:
 *   g++ -std=c++17 -O2 -I../include benchmark_qp_solvers_V3.cpp -o bench_qp
 *   g++ -std=c++17 -O3 -I../include benchmark_qp_solvers_V3.cpp -o bench_qp  # timing
 *
 ...
 *
 * [FIX F1] Batch timing: each rep executes fn() BATCH times and divides
 * total elapsed by BATCH. This overcomes ~1ms clock granularity on systems
 * where a single solve takes only 1–50µs (raw measurement = 0 almost always).
 * Accuracy is taken from the LAST single solve (outside timing loop) so that
 * warm-start solvers are not accidentally pre-warmed by the batch.
 */
static constexpr int TIMING_BATCH = 1000; // [FIX F3c] was 100 — increased to
  // 1000 so that fast solvers (Chol-Direct ~0.3µs, CG-proj S1 ~0.5µs) produce
  // total batch time ~300–500µs, well above ~1ms clock granularity floor.
...
static BenchRow measure(const std::string &sc, const std::string &sv,
                        ...
                        const MPCProblem<QP_N> &mpc, SolveFn fn) {
  std::vector<double> times;
  times.reserve(N_REPS);
  BenchResult<QP_N> sol{};

  // [FIX F1] Batch timing: run TIMING_BATCH solves per rep, divide by batch
  for (int rep = 0; rep < N_REPS; ++rep) {
    auto t0 = Clock::now();
    for (int b = 0; b < TIMING_BATCH; ++b)
      sol = fn(); // last result kept for accuracy check
    auto t1 = Clock::now();
    times.push_back(FpMicros(t1 - t0).count() / TIMING_BATCH);
  }
...
std::vector<BenchRow> run_benchmark() {
  ...
  std::cout
      << "\n╔══════════════════════════════════════════════════════════════╗\n"
      << "║  DCAS Lab QP Solver Benchmark — C++ with CppPlot  [V3]      ║\n"
  ...
  for (auto &sc : scenarios) {
    ...
    Params p_tuned = p_cold;
    p_tuned.rho = rho_opt;
    p_tuned.max_iter = 500; // [FIX F3a] was 200 — with rho=0.5 (clamped) and
    // eps=1e-6/1e-5, S2/S3 need ~300–400 iters. Extra iters are cheap:
    // factorization done once, each iter = O(N²) chol_solve + O(N) clip.
    ...
    for (const auto &x0_pair : sc.x0_list) {
      ...
      // ADMM warm: [FIX F3b] reset() moved INSIDE the lambda so every single
      // call to fn() (including each of the TIMING_BATCH inner reps) starts
      // from a cold state. This gives fair single-solve latency.
      // In V2, reset() was called once before measure() — batch reps 2..100
      // inherited warm state from rep 1 (~1–2 iters) → median = 0µs.
      all_rows.push_back(measure(
          sc.name, "ADMM-warm", pos, vel, g_vec, lb, ub, u_ref, cost_ref, n_act,
          mpc, [&] {
            admm_warm.reset(); // [FIX F3b] reset before every single solve
      ...
      // PGD  [FIX B3] was "DCAS-PGD"
      all_rows.push_back(
          measure(sc.name, "PGD", pos, vel, g_vec, lb, ub, u_ref, cost_ref,
                  n_act, mpc, [&] { return pgd.solve(mpc.H, g_vec, lb, ub); }));

      // CG-proj  [FIX B3] was "CG+Proj"
      all_rows.push_back(
          measure(sc.name, "CG-proj", pos, vel, g_vec, lb, ub, u_ref, cost_ref,
                  n_act, mpc, [&] { return cg.solve(mpc.H, g_vec, lb, ub); }));
...
int main() {
  ...
  std::cout
      << "\n╔══════════════════════════════════════════════════════════╗\n"
      << "║   DCAS QP Solver Benchmark — C++ + CppPlot  [V3]        ║\n"
