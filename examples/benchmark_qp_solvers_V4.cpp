/**
 * @file benchmark_qp_solvers_V4.cpp
 ...
 *
 * [NOTE — NOT FIXED] CppPlot cannot produce boxplots (no ax.boxplot()).
 *   fig_timing shows bar chart of median times instead of box plots.
 *   To add boxplots, ax.bar() would need to be replaced with per-IQR
 *   drawing via SVGBackend.drawRect/drawLine (deferred to CppPlot v2).
 * ─────────────────────────────────────────────────────────────────────────
 *
 ...
 *
 * Compile:
 *   g++ -std=c++17 -O2 -I../include benchmark_qp_solvers_V4.cpp -o bench_qp
 *   g++ -std=c++17 -O3 -I../include benchmark_qp_solvers_V4.cpp -o bench_qp  # timing
 *
...
// ── Adaptive batch sizes (per-solver) ────────────────────────────────────────
// [FIX G2] Single TIMING_BATCH=1000 (V3) still gives median=0 for fast solvers
// (~1–3µs/solve). Per-solver batch ensures total batch time >> clock granularity
// (~1ms) for all solvers while keeping total benchmark runtime acceptable.
//
//  BATCH_FAST  = 10000  → Chol-Direct (~0.3µs), ADMM-warm (~1–3µs)
//                          10000 × 1.5µs = 15ms/rep  >> 1ms granularity ✓
//  BATCH_MED   =  1000  → ADMM-cold (~20µs), ADMM-tuned (~30µs)
//                          1000 × 20µs  = 20ms/rep   >> 1ms granularity ✓
//  BATCH_SLOW  =   200  → PGD (~130µs S3), CG-proj (~100µs S3)
//                          200 × 130µs  = 26ms/rep   >> 1ms granularity ✓
//                          Worst case: PGD S3 200 × 60reps × 20x0 ≈ 31s/scenario
static constexpr int BATCH_FAST = 10000; // Chol-Direct, ADMM-warm
static constexpr int BATCH_MED  =  1000; // ADMM-cold, ADMM-tuned
static constexpr int BATCH_SLOW =   200; // PGD, CG-proj
...
static BenchRow measure(const std::string &sc, const std::string &sv,
  ...
  for (int rep = 0; rep < N_REPS; ++rep) {
    ...
    auto t1 = Clock::now();
    times.push_back(FpMicros(t1 - t0).count() / batch);
  }
...
std::vector<BenchRow> run_benchmark() {
  ...
  std::cout
      << "\n╔══════════════════════════════════════════════════════════════╗\n"
      << "║  DCAS Lab QP Solver Benchmark — C++ with CppPlot  [V4]      ║\n"
  ...
  for (auto &sc : scenarios) {
    ...
    // [FIX G1] Construct p_tuned from scratch (Params{} defaults), NOT from
    // p_cold. p_cold has eps_abs=1e-6, eps_rel=1e-5 (reference quality) which
    // needs ~800–1200 iters with rho=0.5. Default eps_abs=1e-4, eps_rel=1e-3
    // matches recommended MPC config and converges in ~50–150 iters.
    Params p_tuned;               // default: eps_abs=1e-4, eps_rel=1e-3
    p_tuned.rho        = rho_opt; // spectral heuristic, clamped >= 0.5
    p_tuned.max_iter   = 500;     // generous budget; typically ~100 iters used
    ...
    for (const auto &x0_pair : sc.x0_list) {
      ...
      // ADMM cold — medium batch (5–50µs/solve)
      all_rows.push_back(measure(sc.name, "ADMM-cold", pos, vel, g_vec, lb, ub,
                                 u_ref, cost_ref, n_act, mpc, BATCH_MED, [&] {
                                   admm_cold.reset();
                                   return a2b(admm_cold.solve(g_vec, lb, ub));
                                 }));

      // ADMM warm — fast batch; reset inside lambda for fair single-solve timing
      // [FIX F3b] reset() is inside [&]{} so each of the BATCH_FAST inner reps
      // starts cold. This measures first-solve latency (not steady-state).
      all_rows.push_back(measure(
          sc.name, "ADMM-warm", pos, vel, g_vec, lb, ub, u_ref, cost_ref, n_act,
          mpc, BATCH_FAST, [&] {
            admm_warm.reset(); // [FIX F3b] cold start every inner rep
            return a2b(admm_warm.solve(g_vec, lb, ub));
          }));

      // ADMM tuned — medium batch (30–60µs/solve)
...
//   NOTE: ax.bar() + set_yscale("log") → bars invisible (CppPlot limitation).
//         fig_timing uses vertical stem lines (ax.plot) instead. [FIX F3d]
// ══════════════════════════════════════════════════════════════════════════════

static const std::string SC_NAMES[] = {"S1_easy", "S2_medium", "S3_hard"};
static const std::string SC_SHORT[] = {"S1: Easy", "S2: Medium", "S3: Hard"};

// ── Fig 1: Timing bar chart (stem plot)
// ─────────────────────────────────────
/**
 * [FIX F3d] ax.bar() + set_yscale("log") → bars invisible (CppPlot limitation:
 *   bars are rendered at linear coordinates before log scale is applied).
 *   Fix: replace ax.bar() with per-solver vertical stem lines drawn as
 *   2-point ax.plot({x,x}, {y_floor, y_val}) on a log Y axis.
 *   Each stem has a unique colour matching its legend entry.
 *   y_floor = 0.3µs (below minimum visible value of 0.5µs) ensures stems
 *   start from the bottom of the log axis.
 *
 * [FIX P1/P2 retained] Y-axis log scale via set_yscale("log").
 *   Solver names via legend (CppPlot has no set_xticklabels()).
 */
void fig_timing(const std::vector<BenchRow> &rows, const std::string &out) {
  Figure fig(1400, 540);

  // Per-solver colours: stem line + legend marker (line style "-" needed for
  // legend to show a coloured line, not just a dot)
  const std::vector<std::string> stem_fmts = {
      "r-",  // ADMM-cold    red
      "b-",  // ADMM-warm    blue
      "m-",  // ADMM-tuned   magenta
      "g-",  // PGD          green
      "c-",  // CG-proj      cyan
      "k-"   // Chol-Direct  black
  };

  static constexpr double Y_FLOOR = 0.3; // stem base on log axis
  ...
  for (int si = 0; si < 3; ++si) {
    ...
    for (int k = 0; k < (int)SOLVER_ORDER.size(); ++k) {
      auto t = collect(rows, sc, SOLVER_ORDER[k],
                       [](const BenchRow &r) { return r.t_median_us; });
      if (t.empty())
        continue;
      double tmed  = median_v(t);
      double y_val = std::max(tmed, 0.5); // floor at 0.5µs
      double x_pos = k + 1.0;
      ymax = std::max(ymax, y_val);

      // [FIX F3d] Vertical stem: 2-point line from Y_FLOOR to y_val
      ax.plot({x_pos, x_pos}, {Y_FLOOR, y_val},
              stem_fmts[k % stem_fmts.size()].c_str(),
              {{"label", std::string(SOLVER_ORDER[k])}});

      // P95 tick: short horizontal cap at the top of the stem
      auto tp = collect(rows, sc, SOLVER_ORDER[k],
                        [](const BenchRow &r) { return r.t_p95_us; });
      if (!tp.empty()) {
        double p95 = std::max(median_v(tp), 0.5);
        ax.plot({x_pos - 0.15, x_pos + 0.15}, {p95, p95},
                stem_fmts[k % stem_fmts.size()].c_str(), {});
      }
    }
    ...
    // Reference lines: 1ms and 10ms WCET deadline
    ...
    ax.set_title(SC_SHORT[si] + " — Median Solve Time  (cap = P95)");
    ax.set_xlabel("Solver  (see legend)");
    ax.set_ylabel("Time (us)");
    ax.set_xlim(xlo, xhi);
    ax.set_ylim(Y_FLOOR, std::max(ymax * 5.0, 31600.0));
    ax.grid(true);
    ax.legend(true);
  }
  fig.savefig(out);
...
void write_summary(const std::vector<BenchRow> &rows, const std::string &path) {
  ...
  for (auto &sn : SOLVER_ORDER) {
    ...
    for (auto &sc : {"S1_easy", "S2_medium", "S3_hard"}) {
      ...
      ss << std::fixed << std::setprecision(0) << median_v(tm) << "/"
         << median_v(tp) << "/" << *std::max_element(tx.begin(), tx.end());
...
int main() {
  ...
  std::cout
      << "\n╔══════════════════════════════════════════════════════════╗\n"
      << "║   DCAS QP Solver Benchmark — C++ + CppPlot  [V4]        ║\n"
