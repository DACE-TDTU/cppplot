...
//
//  Compile:   g++ -std=c++17 -O2 -I../include demo_mpc_trajectory.cpp -o demo_traj && ./demo_traj
...
void fig_exp3_trajectory(const std::string& out) {
    ...
    // ── Plot: 3 cols × 3 rows ───────────────────────────────
    // Layout: subplot(3, 3, idx)  idx = 1..9  (row-major)
    //   Row 0 (pos):   idx = 1, 2, 3
    //   Row 1 (vel):   idx = 4, 5, 6
    //   Row 2 (ctrl):  idx = 7, 8, 9
    //
    // Confirmed CppPlot API used:
    ...
    // Colors: unconstrained=blue, light=orange(red), tight=purple(magenta)
    ...
    for (int col = 0; col < 3; ++col) {
        ...
        // ── Row 0 (idx = col+1): Position ───────────────────
        {
            auto& ax = fig.subplot(3, 3, col + 1);
            ...
            ax.plot({t0, tN}, {X_REF, X_REF}, "y--",
                    {{"label", std::string("reference = ") + std::to_string(X_REF)}});
        ...
        // ── Row 1 (idx = col+4): Velocity ───────────────────
        {
            auto& ax = fig.subplot(3, 3, col + 4);
        ...
        // ── Row 2 (idx = col+7): Control input ──────────────
        {
            auto& ax = fig.subplot(3, 3, col + 7);

            ax.plot(s.t_vec, s.u_vec, fmt_line[col].c_str(),
                    {{"label", std::string("u")}});

            // ±u_lim bound lines
            std::string lim_label = std::string("\xc2\xb1")
                                  + std::to_string(u_lim).substr(0, 4);
            ...
            ax.set_xlabel("Time [s]");
            ax.set_ylabel("Control [m/s\xc2\xb2]");
...
// ============================================================
int main() {
    printf("=== demo_mpc_trajectory.cpp ===\n");
