 ...
 * @brief Demonstration of μ-analysis (structured singular value) for robust
 * control
 *
 * Shows D-scaling upper bound, μ frequency sweep, and robust performance
 * analysis. compile g++ -std=c++17 -D_USE_MATH_DEFINES -I../include
 * mu_analysis_demo.cpp -o mu_analysis_demo run ./mu_analysis_demo
 */
 ...
 */
double muUpperBound_full(const CMatrix2x2 &M) { return M.maxSingularValue(); }
 ...
 */
CMatrix2x2 buildMMatrix(const TransferFunction &G, const TransferFunction &K,
                        const TransferFunction &Wp, const TransferFunction &Wu,
                        double omega) {
...
╚══════════════════════════════════════════════════════════════════════╝
  ...
  CMatrix2x2 M(Complex(0.5, 0.1), Complex(0.2, -0.1), Complex(0.3, 0.2),
               Complex(0.4, 0.0));
  M.print("M");
...
╚══════════════════════════════════════════════════════════════════════╝
  ...
  std::vector<double> freqs = {0.1,  0.5,  1.0,  2.0,  5.0,
  ...
  for (double omega : freqs) {
    ...
    std::cout << "  " << std::setw(8) << omega << "   " << std::setw(5)
              << std::fixed << std::setprecision(3) << S_mag << "    "
              << std::setw(5) << T_mag << "    " << std::setw(6) << WpS << "   "
              << std::setw(6) << WuT << "    " << std::setw(5) << mu << "\n";
  }

  std::cout << "\n----- Robust Performance Result -----\n";
  std::cout << "  Peak μ = " << mu_peak << " at ω = " << omega_peak
            << " rad/s\n";
...
----- Simplified Example -----
  ...
  for (size_t i = 0; i < freqs.size(); i++) {
    ...
    d_opt[i] = d_best;
    std::cout << "  " << std::setw(5) << omega << "    " << std::setw(5)
              << std::setprecision(3) << mu_min << "    " << std::setw(5)
              << d_best << "\n";
  }
...
----- Design Guidelines for Motor Control -----
  ...
  if (mu_max < 1.0) {
    ...
    std::cout
        << "  Consider: reducing bandwidth, increasing controller order,\n";
