 ...
 *   g++ -std=c++17 -O3 -I../include benchmark_qp_solvers.cpp -o bench_qp  #
 * timing
 *
...
std::vector<BenchRow> run_benchmark() {
  ...
  std::cout
      ...
      << "╚══════════════════════════════════════════════════════════════╝\n"
      << "  N_REPS=" << N_REPS << "  N_WARMUP=" << N_WARMUP << "  QP_N=" << QP_N
      << "\n\n";
...
void fig_timing(const std::vector<BenchRow> &rows, const std::string &out) {
  ...
  for (int si = 0; si < 3; ++si) {
    ...
    for (int k = 0; k < (int)SOLVER_ORDER.size(); ++k) {
      ...
      std::vector<double> t_floored;
      for (double val : t) {
        double y_val = std::max(val, 0.5); // floor at 0.5µs
        t_floored.push_back(y_val);
        ymax = std::max(ymax, y_val);
      }

      box_data.push_back(t_floored);
      box_pos.push_back(k + 1.0);
      box_labels.push_back(std::string(SOLVER_ORDER[k]));
    }

    ax.boxplot(box_data, box_pos);
    ax.set_xticklabels(box_labels, box_pos);
    ax.set_yscale("log");
