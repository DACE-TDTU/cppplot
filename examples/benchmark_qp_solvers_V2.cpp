/**
 * @file benchmark_qp_solvers_V2.cpp
 ...
 *   Fix: batch timing — each rep executes fn() BATCH=100 times, total time
 *   divided by BATCH. Accuracy measurement still uses the last single solve.
 *   Location: measure() function.
 *
 * [FIX F2] CRITICAL — ADMM-tuned never converges (iters=200, converged=0).
 *   spectral_rho() returns values << 1 for S2/S3 (e.g. rho≈0.15 for S2,
 *   rho≈0.06 for S3) because lambda_max(H) << 1. With rho<<1, ADMM penalty
 *   is too weak → primal/dual residuals stall above eps threshold within
 *   max_iter=200. ADMM-tuned ends up WORSE than ADMM-cold (rho=1.0).
 *   Fix: clamp rho_opt from below at 0.5 in spectral_rho(). This ensures
 *   ADMM-tuned always has at least as strong a penalty as needed for
 ...
 *
 * Compile:
 *   g++ -std=c++17 -O2 -I../include benchmark_qp_solvers_V2.cpp -o bench_qp
 *   g++ -std=c++17 -O3 -I../include benchmark_qp_solvers_V2.cpp -o bench_qp  # timing
 *
 ...
 */
static constexpr int TIMING_BATCH = 100; // inner repeat count for timing
...
std::vector<BenchRow> run_benchmark() {
  ...
  std::cout
      << "\n╔══════════════════════════════════════════════════════════════╗\n"
      << "║  DCAS Lab QP Solver Benchmark — C++ with CppPlot  [V2]      ║\n"
  ...
  for (auto &sc : scenarios) {
    ...
    for (const auto &x0_pair : sc.x0_list) {
      ...
      // ADMM warm: reset once before the entire measure() call so the batch
      // accumulates warm-start state across its TIMING_BATCH inner reps —
      // this correctly models closed-loop steady-state (the hot-path case).
      admm_warm.reset();
      all_rows.push_back(measure(
          sc.name, "ADMM-warm", pos, vel, g_vec, lb, ub, u_ref, cost_ref, n_act,
          mpc, [&] { return a2b(admm_warm.solve(g_vec, lb, ub)); }));
...
// ───────────────────────────────────────────────────
/**
 * [FIX P1] Y-axis uses native log scale via ax.yscale("log").
 *   Reference lines: 1000 (=1ms), 10000 (=10ms deadline).
 *
 * [FIX P2] Solver names absent from X axis (only integers 1..6 shown).
 *   Root cause: CppPlot ax.bar() has no set_xticklabels(); ax.set_xlabel()
 *   can only set one string for the entire axis.
 *   Fix: add one ax.plot({k+1},{y}, fmt, label=solver) dummy call per solver
 *   so legend() reveals solver identity for each bar position.
 */
void fig_timing(const std::vector<BenchRow> &rows, const std::string &out) {
  Figure fig(1400, 540);

  // Per-solver format strings — used for legend identity markers
  const std::vector<std::string> fmts = {"r-", "b-", "m-", "g-", "c-", "k-"};
  ...
  for (int si = 0; si < 3; ++si) {
    ...
    for (int k = 0; k < (int)SOLVER_ORDER.size(); ++k) {
      ...
      // [FIX P2] dummy 1-point plot gives this bar a legend entry
      ax.plot({k + 1.0}, {y_val}, fmts[k % fmts.size()].c_str(),
              {{"label", std::string(SOLVER_ORDER[k])}});
    }

    ax.bar(xs, ys); // bars on top of (hidden) dummy points
    ax.set_yscale("log");

    // Reference lines: 1ms and 10ms deadline
    double xlo = 0.3, xhi = (double)SOLVER_ORDER.size() + 0.7;
    ax.plot({xlo, xhi}, {1000.0, 1000.0},
            "m:", {{"label", std::string("1ms")}});
    ax.plot({xlo, xhi}, {10000.0, 10000.0}, "r--",
            {{"label", std::string("10ms deadline")}});

    ax.set_title(SC_SHORT[si] + " — Median Solve Time");
    ax.set_xlabel("Solver index (see legend for names)");
    ax.set_ylabel("Median time, us");
    ax.set_xlim(xlo, xhi);
    ax.set_ylim(0.5, std::max(ymax * 4.0, 31600.0)); // 10^4.5 approx 31622
    ax.grid(true);
    ax.legend(true);
  }
  fig.savefig(out);
...
int main() {
  ...
  std::cout
      << "\n╔══════════════════════════════════════════════════════════╗\n"
      << "║   DCAS QP Solver Benchmark — C++ + CppPlot  [V2]        ║\n"
